...
public final class Lightmap {
    ...
    private boolean compute(EntityRenderer renderer, World world, int[] mapRGB, float partialTick) {
        float sun = ColorUtils.clamp(world.lightningFlash > 0 ? 1.0f : 7.0f / 6.0f * (world.getSunAngle(1.0f) - 0.2f)) * (width - 1);
        ...
        float gamma = ColorUtils.clamp(Minecraft.getInstance().gameSettings.gammaSetting);
